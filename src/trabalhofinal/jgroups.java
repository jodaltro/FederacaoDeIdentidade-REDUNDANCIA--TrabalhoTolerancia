	...
	private void eventLoop() {
		...
		while (true) {
			...
			if (last != 0) {
				...
				if (msg_recebida[0].equals("host")) {
					cont_s = Integer.parseInt(msg_recebida[3]);
					if (cont_s == cont) {
						usuario = msg_recebida[1];
						senha = msg_recebida[2];
						...
						cont++;
						System.out.println("Usuaro cadastrado na redundancia!");
					}
		 ...
		 * InputStreamReader(System.in)); while(true) { try { System.out.print(
		 * "> "); System.out.flush(); String line=in.readLine().toLowerCase();
		 ...
		 * 
		 * 
		 * for (int x = 0; x < Nuvem.nos.size(); x++) { System.out.println(
		 * "***No; " + Nuvem.nos.get(x).getIdAux()); System.out.println("-- " +
		 * Nuvem.nos.get(x).getID() + " " + Nuvem.nos.get(x).getAdress());
		 * System.out.println("-->" +
		 * Nuvem.nos.get(x).getNo().printFingerTable()); System.out.println("->"
		 * + Nuvem.nos.get(x).getNo().printEntries()); System.out.println("\n");
